  ...
  for (const stmt of sf.statements) {
    if (ts.isVariableStatement(stmt)) {
      for (const decl of stmt.declarationList.declarations) {
        if (decl.name && ts.isIdentifier(decl.name) && decl.name.text === 'TASK_CATALOG_I18N') {
          dataStmt = stmt;
          break;
        }
      }
...
async function main(): Promise<void> {
  ...
  console.log(`[task-catalog-generate] extracted ${taskCount} en task entries from ${catalogPath}`);
  if (taskCount === 0) {
