...
**IMU rates & video↔IMU drift** *(quality telemetry)*
| Field | Type | Notes |
|---|---|---|
...
**Video spec** *(everything below is measured from the actual encoded file, not hardcoded)*
| Field | Type | Notes |
|---|---|---|
...
### `calibration` *(schema 1.2.0 — quick 260522-elm; additive telemetry)*
...
calibration *adds* offset/extrinsics telemetry alongside them.
...
|---|---|---|
| `timestamp_ns` | int64 | nanoseconds, monotonic clock (`elapsedRealtimeNanos`). **Not** wall-clock — use it for *relative* timing/alignment, not calendar time. |
...
*Indexes:* unique on `google_sub`; index on `deleted_at`.
...
*Indexes:* unique `slug`; `category`; HNSW on `embedding`; GIN on `name_search`.
...
*Indexes:* `(user_id, captured_at)`, `qa_status`, `task_id`.
...
| Only *usable* recordings | `recordings WHERE qa_status = 'verified'` |
...
2. **Drift is telemetry, not a gate.** `imu_video_drift_*` of ~1.7–6.2 ms is *expected and
   acceptable* — the ultrawide lens causes it. Do **not** flag recordings as bad on drift.
...
3. **Cancelled & practice segments never reach the server.** If a recording dropped below
   ...
   good captures** — to study capture *failures* you'd need on-device telemetry/`events`,
   not `recordings`.
