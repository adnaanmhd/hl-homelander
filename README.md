...
**Upload pipeline** — the server side (multipart `recordings` routes, presigned URLs, S3 lifecycle) and the client-side `HumynUpload` native module shipped in **Phase 5** (2026-05-13). _(The BullMQ hash-verify worker that was originally part of Phase 5 was removed 2026-06-04 — Enh 3 / D1; uploads now finalize directly to `qa_status = 'uploaded'`, the terminal success state, and the device deletes its local copy on a `/finalize` 200. See `UPLOAD-PIPELINE.md`.)_
...
| **3 — HumynCapture Native Module**                              | Camera2 + MediaCodec HEVC + IMU CSV + metadata JSON with timestamp alignment, drift, segmentation (audio dropped 2026-05-11; device-side hashing later removed — Enh 3 / D1, 2026-06-04)                                 | ✅ done (2026-05-11) |
...
| **5 — Upload Pipeline**                                         | `HumynUpload` (multipart S3, Android 14/15 FGS-type-downgrade + UIDT JobService). _(The BullMQ hash-verify worker, per-account rate cap, and fraud dashboard were removed / descoped — Enh 3 / D1 + FRAUD-05/06 → §v2.)_ | ✅ done (2026-05-13) |
| **6 — Tasks, History, Home Tiles & Lexical Search**             | Task catalogue + server-side `ts_vector` search, History grouped by day with in-app player, Home dynamic tiles                                                                                                           | ✅ done              |
| **7 — Multi-linguality & Live-Cam Feed**                        | 8 locales (UI copy / device-TTS cues / locale-aware dates, chosen at first launch, changeable in Profile) + a live ultrawide preview during recording (15 s on Start, 10 s on tap)                                       | ✅ done (2026-05-27) |
| **8 — Observability & APK Distribution Hardening**              | Firebase Analytics funnel, Crashlytics, structured CloudWatch logs, signed-APK pipeline hardening _(the Bull-Board hash-verify dashboard was dropped — worker removed, Enh 3 / D1)_                                      | ⏳                   |

**Descoped from the MVP** (parked in `.planning/REQUIREMENTS.md` §v2): semantic / pgvector + RRF hybrid search on the client surface (backend layer shipped, just not surfaced), the server-side IMU-liveness fraud check, audio capture, the upload verification / hash-verify worker (removed 2026-06-04 — Enh 3 / D1), the per-account daily upload-rate cap, the Play Store staged rollout, and the entire iOS App Store channel + iOS native-module analogues. MVP anti-fraud = Play Integrity at sign-in + the on-device one-shot hand gate. MVP geos = India + Brazil (English-first; 8 UI locales shipped in Phase 7).

**Post-MVP bug-fix + enhancement pass** (2026-06-04, branch `fix/bugs-enhancements-260604`) — 11 reported bugs + 3 enhancements, including: removing the upload verification flow + all hashing so `uploaded` is terminal success (D1); single-device, newest-login-wins auth that overrides the stateless-JWT decision (D2); **precise GPS location per recording with an updated consent + a consent-version bump** (D3); backend-generated poster thumbnails for cross-device History (D5); a 3-minute per-segment capture floor (D6); and server-persisted practice-tutorial completion (D7). See `IMPLEMENTATION-PLAN-260604.md`.
