...
**Impact on plan:** None of the auto-fixes change the contract the plan describes. The acceptance criteria (file existence + grep counts + green vitest cases + telemetryRing.ts untouched) all hold; only the _paths_ and the _cross-validation source_ moved to match the actual project layout + the parallel-wave file-delivery reality. No scope creep.
