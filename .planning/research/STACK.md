...
### Backend — Hash-Verify Worker Queue

| Technology            | Version (Pin) | Purpose                                                                                        | Confidence | Why this version                                                                                            |
| --------------------- | ------------- | ---------------------------------------------------------------------------------------------- | ---------- | ----------------------------------------------------------------------------------------------------------- |
| `bullmq`              | **5.76.8**    | Hash-verify worker queue — retries/backoff/concurrency/queue-depth metrics for ECS autoscaling | HIGH       | ROADMAP locks BullMQ-on-Redis-on-ECS for VERIFY-01/07; switching to S3-EventBridge→Lambda is a §v2 concern. |
| `ioredis`             | **5.10.1**    | Redis client BullMQ rides on                                                                   | HIGH       | BullMQ's required Redis client.                                                                             |
| `@aws-sdk/client-sqs` | **3.1044.0**  | Thin SQS-poller in prod (S3→EventBridge→SQS→`queue.add`)                                       | HIGH       | AWS SDK v3 modular, same minor as `@aws-sdk/client-s3` per the "always same minor" rule.                    |
| **Redis**             | **7.x**       | Queue backend (ElastiCache prod; `redis:7-alpine` container in dev)                            | HIGH       | Redis 7+ is the BullMQ baseline.                                                                            |

> **CLAUDE.md Redis carve-out:** "Do NOT Use → Redis for the on-device upload queue" — the _upload_ queue lives on device (MMKV-backed). The _hash-verify worker_ queue is the one Redis carve-out at MVP (BullMQ-on-Redis-on-ECS). No other Redis usage.
...
| **Redis for the on-device upload queue** at MVP                                                         | Adds infra; the upload queue lives on the device anyway, and the backend is stateless per request. (The _hash-verify worker_ queue IS the one Redis carve-out — BullMQ-on-Redis-on-ECS per VERIFY-01/07; see "Backend — Hash-Verify Worker Queue" above.)                   | Postgres + Drizzle for everything except the worker queue. Redis 7.x only for the BullMQ hash-verify worker. |
