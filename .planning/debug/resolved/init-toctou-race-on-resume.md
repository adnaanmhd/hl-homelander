---
status: resolved
resolution: "Bug-1 fix landed at commit `59e97f3` — Shape A (try/catch INSERT → `isUniqueViolation('recordings_pkey')` → AbortMultipartUploadCommand on orphan video+IMU → re-fetch row → fall through to `replyExistingRowIdempotent` helper that the SELECT-first guard takes; ~95-LOC helper extracted from the existing-row block + ~40 LOC of try/catch wiring + the AbortMultipartUploadCommand import). Backend vitest at apps/api/test/routes/recordings/init.test.ts pins the race contract: two concurrent /init for the same recordingId NEVER both 500; SEQUENTIAL second /init AFTER the first commits still returns 200 idempotent (preserves CR-02 lost-201 self-heal). Validated end-to-end on §4a re-walk recording `01KRGK1DJ1HTW31FN33B07EGMP` (Pixel 10a, post-fix build): drainer fired second /init → race-loss → `replyExistingRowIdempotent` returned 200 → upload completed → /finalize 200 → worker verify at 11:59:55 → events delivered at 12:01:46 → clearVerified → locals deleted → queue row dropped → contributions row written. §4b (User-B isolation) + §4c (switch back to Adnaan) also PASSED on the post-fix build. UAT Item 6 result flipped `partial → pass`; severity `partial-step1-passed-step2-blocked → passed`. Bug-2 secondary (UploadCoordinator.kt:213-217 transient-failure has no in-loop retry + Wave-1.5 Item 4 tile-tap kick didn't fire) flagged for Wave-2 (does NOT block §4; Bug-1 fix alone resolves §4 because post-fix /init never returns 500 for the race). Stuck DB row `01KRGGEDK65J5HE6BNDKXPZ1DQ` from BLOCKED-finding walk preserved as historical evidence."
resolved_at: 2026-05-13T12:45:00Z
