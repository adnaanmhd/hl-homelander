---
status: resolved
resolved: 2026-05-12T02:40:00Z
resolution_note: 'RESOLVED. Fix = the reanimated 4.3.1 bump (+ react-native-worklets 0.8.3, babel.config.js → react-native-worklets/plugin, CLAUDE.md pin updated) — commit 57ed029; the build-wall lift also surfaced a HandLandmarkerOptions-nesting import bug fixed in d97637a; the separate Crashlytics-Gradle-plugin-missing crash was fixed in e9ab59d. Build green (./gradlew installApkRolloutDebug BUILD SUCCESSFUL), vitest + tsc clean. Runtime confirmation: during the 2026-05-12 on-hardware smoke walk (04-MANUAL-SMOKE §2) PracticeCompleteScreen rendered on-device with its confetti + badge scale-pop animations (the two files that import reanimated 4 — PracticeCompleteScreen.tsx, components/Confetti.tsx) — reanimated 4 works at runtime on RN 0.83. Moved to .planning/debug/resolved/.'
