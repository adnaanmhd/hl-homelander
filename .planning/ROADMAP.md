...
- [x] **Phase 5: Upload Pipeline & Hash-Verify Worker** - S3 multipart with UIDT JobService + URLSession bg, hash-verify worker on BullMQ, server→client verified/re-upload event channel (server-side IMU-liveness check FRAUD-03/04 and the FRAUD-05/06 anti-fraud surface deferred to v2 — see REQUIREMENTS.md §v2). **Wave 1 = the `04-COSMETIC-GAPS.md` cosmetic-cleanup pass** (doc polish + the alert-tone re-check / device-distress-stop-navigation / defer-the-recovery-toast-to-Home code items), run before the upload work — mirrors how `02-COSMETIC-GAPS.md` was Phase 3's Wave 1. (completed 2026-05-13 evening; UAT walked end-to-end on Pixel 10a — Items 1+2+4+5+6 PASS, Item 3 PARTIAL with the OEM device sweep carried forward to Phase 8; all 15 plans landed (05-01..05-15); 8 evidence recordings preserved in DB; cellular Item 4 walk via Jio CGNAT + cloudflared tunnels recorded `01KRH652NZ0TRF645KNR0K4440` qa_status='verified' at 17:44:31 UTC; new cosmetic finding `HumynBeep`/SoundPool tones + haptics silent on Android 16 folded into Phase 6 — see ROADMAP §Phase 6 carry-over)
- [x] **Phase 6: Tasks, History, Home Tiles & Lexical Search** - 65-task catalog with server-side lexical (`ts_vector` + GIN) task search, History grouped by day with in-app player, Home dynamic tiles with time-range filters (completed 2026-05-14; all 11 main plans + Plan 06-12 cosmetic cleanup + a Plan-06-12 follow-on wave landed; 06-MANUAL-SMOKE.md §7 sign-off YES on Pixel 10a; all 15 findings in 06-COSMETIC-GAPS.md resolved except 4 (HumynBeep audibility — owner-deferred per `feedback_d09_audibility_deferred`) and 9 (Player byte-0 seek — needs the Phase-8 finalize-time fmp4 → flat-MP4 remux); full E2E happy path + 4 negative scenarios (compat fail / thermal pre-flight + mid-record / battery <15% / battery <5%) walked clean 2026-05-14)
- [x] **Phase 7: Multi-linguality & Live-Cam Feed** - 8 languages (English + pt-BR + es + hi-IN + bn-IN + ta-IN + te-IN + mr-IN) end-to-end across UI copy / device-TTS audio cues / locale-aware date formatting, with the language chosen once at first launch on a new Choose-Language screen and changeable in Profile; live ultrawide camera feed during recording (15 s on Start, then 10 s on tap, rolling timer) without regressing the recorded `imu_video_drift_{max,mean,p99}_ms` telemetry or the fps/resolution/insufficient_frames cancel gates (inserted 2026-05-24; **completed 2026-05-27** — 17 plans landed across the original cluster + Plan 07-16 (G-13..G-28 closure) + Plan 07-17 (G-30..G-40 re-walk fix cluster: Button primitive `width:'100%'`, HistoryRow `localizeTaskName` wire, content-hugging stretch fixes on CompatRunning/RotatePrompt/liveEyeHint, `numberOfLines={1}` on Tasks pills + Report chips + HomeHero CTA, reverseSearch Stage 1.5 substring shim); Plan 07-15 superseded by 07-17's operator walk; operator-walked 7-locale hardware deep walk on APK `9dbb1d5` on Pixel 10a `5C161JEA304304` returned ALL PASS — `i'm happy with all languages, push the changes`)
- [ ] **Phase 8: Observability & APK Distribution Hardening** - Firebase Analytics funnel, Crashlytics, structured CloudWatch logs, ~~Bull-Board hash-verify dashboard~~ (**dropped 2026-06-04 — the hash-verify worker was removed, Enh 3 / D1; no queue left to dashboard**), signed-APK distribution pipeline production-hardened (iOS parity + staged Play Store / App Store rollout deferred — see REQUIREMENTS.md §v2) (renumbered from Phase 7 2026-05-24 — content unchanged)
...
> **⚠ Hash-verify half REMOVED 2026-06-04 (Enh 3 / D1 — owner sign-off `.planning/260604-locked-override-signoff.md`).** The upload-pipeline half of this phase stands; the **verification half is gone**. Deleted: the BullMQ/Redis/SQS/EventBridge hash-verify worker, the device-side + server-side SHA-256 of MP4 + IMU CSV, the `verified` / `re-upload` event channel, `recordings.file_sha256` / `imu_sha256` / `verified_at`, and the `recordings_to_verify` + `recording_events_outbox` tables (migration 0011). **`uploaded` is now the terminal success state**; the device deletes its local files on a `/finalize` 200. The Goal / Success-Criteria #4 / VERIFY-01..07 / the 05-03 / 05-05 / 05-12 / 05-13 sub-plans below are retained as **historical record** — they describe what was built, not current behavior. Trail: `IMPLEMENTATION-PLAN-260604.md` §6, `UPLOAD-PIPELINE.md`, REQUIREMENTS.md §"Backend Hash-Verify Worker".
...
**Goal**: The app ships in 8 languages (English + Portuguese + Spanish + Hindi + Bengali + Tamil + Telugu + Marathi) end-to-end across UI copy, audio cues, and locale-aware date formatting, with the language chosen once at first launch and changeable in Profile; and during recording the operator can briefly see the live ultrawide camera feed (15 s automatically after Start, then 10 s on tap) without regressing the LOCKED capture spec or the recorded `imu_video_drift_{max,mean,p99}_ms` telemetry.
**Depends on**: Phase 6
**Requirements**: I18N-01, I18N-02, I18N-03, I18N-04, I18N-05, I18N-06, I18N-07, I18N-08, I18N-09, I18N-10, I18N-11, I18N-12, REC-LIVE-01, REC-LIVE-02, REC-LIVE-03, REC-LIVE-04, REC-LIVE-05, REC-LIVE-06, REC-LIVE-07
**Success Criteria** (what must be TRUE):

1. On a fresh install on a Pixel 7a/8a/10a-class Android device, the user can: complete Splash → Choose Language (selecting any of the 8 languages) → Sign-up (consent rendered bilingual when non-English) → Permissions → Compat → Tutorial → Home → Tasks → record a segment → see History → open Profile and change language → see the new language take effect immediately across all 23 screens (22 from Phase 2 + the new picker)
2. During a 10-minute recording on a Pixel 10a with the live-cam feed feature: (a) the initial 15 s preview renders the ultrawide stream at system brightness, (b) tap-to-reveal works after the dim transition, (c) the recorded segment's metadata JSON still carries valid `imu_video_drift_{max,mean,p99}_ms` values within 50% of the no-preview baseline (current ~1.7–6.2 ms range), (d) the capture-quality cancel gates (fps/resolution/insufficient_frames) still fire correctly post-encode, (e) the segment uploads end-to-end via Phase 5 with `qa_status = 'verified'`
3. The audio cues for the active locale play via device TTS in the picked language on a device that has the engine installed (verify on Pixel 10a for pt-BR / es / hi-IN / bn-IN / ta-IN / te-IN / mr-IN); on a locale with no engine present, English fallback plays and the fallback is logged to Crashlytics
...
- [x] 07-05-screen-string-sweep-and-bilingual-consent-PLAN.md — 22-screen t() sweep + bilingual TermsOfUseModal + API-error→translated toast + LLM regen of 7 non-English JSONs (I18N-07/08/09/11/12)
- [x] 07-06-tts-fallback-and-reverse-search-PLAN.md — 5-step TTS fallback chain + taskCatalog.i18n.ts + 3-stage reverseSearch + tasksApi.ts shim (I18N-06, I18N-10)
...
- [x] 07-11-i18n-sweep-extension-PLAN.md — CompatCheck labels + RotatePrompt + RecordingScreen TTS cue text + HomeHero greeting + tab labels + History time-filter chips + UploadStatusChip (G-02/03/04/05/06/07/09); en.json + 7 non-en regen (I18N-01, I18N-11)
- [x] 07-13-help-center-body-translation-PLAN.md — new sibling content.{locale}.json files for the Help Center body via the LLM tool; contentLoader.ts wires the locale select (G-10; D-03) (I18N-01)
- [x] 07-14-COSMETIC-PLAN.md — Signup consent center-align (COSMETIC-01) + Live-preview label top-left (COSMETIC-02) + Eye glyph accent color (COSMETIC-03); 07-COSMETIC-GAPS.md ledger flipped to closed (I18N-01, REC-LIVE-01, REC-LIVE-03)

Internal Wave B (depends on Wave A 07-11 catalog stability):

- [x] 07-12-task-catalog-body-translation-PLAN.md — new tools/i18n/task-catalog-generate.ts LLM regen for all 77 tasks × 7 non-en locales body translation (G-08; D-01); reverseSearch Stage 1+2 become functionally meaningful (I18N-01, I18N-10)
