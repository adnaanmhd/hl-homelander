...
5. The backend enforces a per-account daily upload-rate cap as a coarse fraud heuristic, and the pre-payout fraud monitoring dashboard surfaces hash-mismatch rate + account-fingerprint clustering + OEM/region anomalies. _(The server-side IMU-liveness check and its `liveness_score` panel — FRAUD-03/04 — are deferred to §v2 as of 2026-05-11; see `imu-liveness-check.md` and `deferred-decisions.md` for the v2 design.)_
   **Plans**: TBD
