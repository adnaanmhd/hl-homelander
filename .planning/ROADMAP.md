...
- [ ] **Phase 5: Upload Pipeline, Hash-Verify Worker & Anti-fraud** - S3 multipart with UIDT JobService + URLSession bg, hash-verify worker on BullMQ, per-account upload-rate cap + pre-payout fraud dashboard (server-side IMU-liveness check FRAUD-03/04 deferred to v2 — see REQUIREMENTS.md §v2)
