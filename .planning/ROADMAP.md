...
- [ ] **Phase 5: Upload Pipeline, Hash-Verify Worker & Anti-fraud** - S3 multipart with UIDT JobService + URLSession bg, hash-verify worker on BullMQ, per-account upload-rate cap + pre-payout fraud dashboard (server-side IMU-liveness check FRAUD-03/04 deferred to v2 — see REQUIREMENTS.md §v2). **Wave 1 = the `04-COSMETIC-GAPS.md` cosmetic-cleanup pass** (doc polish + the alert-tone re-check / device-distress-stop-navigation / defer-the-recovery-toast-to-Home code items), run before the upload work — mirrors how `02-COSMETIC-GAPS.md` was Phase 3's Wave 1. Also surface in the plan proper: the upload-bundle validation / hash-verify worker must tolerate a crash-recovered segment's `duration_seconds: 0` + null `imu_video_drift_*` / `imu_min_rate_*` (those can't be recomputed from a crash-truncated partial — see `04-COSMETIC-GAPS.md` "New from the 2026-05-12 re-walk").
...
**Goal**: A finished recording uploads automatically as a three-file bundle, survives backgrounding / force-quit / Android 14/15 service-type restrictions / OEM battery managers, gets re-hashed server-side and either confirmed via a `verified` event or queued for re-upload, with a per-account daily upload-rate cap and the pre-payout fraud-monitoring dashboard standing up the coarse anti-fraud surface before payouts go live. _(The server-side IMU-liveness check — FRAUD-03/04 — was briefly in this phase, then descoped to §v2 on 2026-05-11; the upload bundle still carries the IMU CSV, it is just not analysed server-side at MVP.)_
**Depends on**: Phase 3
**Requirements**: UP-01, UP-02, UP-03, UP-04, UP-05, UP-06, UP-07, UP-08, UP-09, UP-10, UP-11, UP-12, UP-13, UP-14, UP-15, UP-16, UP-17, UP-18, UP-19, VERIFY-01, VERIFY-02, VERIFY-03, VERIFY-04, VERIFY-05, VERIFY-06, VERIFY-07, FRAUD-05, FRAUD-06
**Success Criteria** (what must be TRUE):
...
5. The backend enforces a per-account daily upload-rate cap as a coarse fraud heuristic, and the pre-payout fraud monitoring dashboard surfaces hash-mismatch rate + account-fingerprint clustering + OEM/region anomalies. _(The server-side IMU-liveness check and its `liveness_score` panel — FRAUD-03/04 — are deferred to §v2 as of 2026-05-11; see `imu-liveness-check.md` and `deferred-decisions.md` for the v2 design.)_ **NOTE (planning, 2026-05-12):** D-04 descopes ALL anti-fraud to §v2 — Plan 05-02 (Wave 1) retires FRAUD-05/06 to §v2, re-titles this phase "Upload Pipeline & Hash-Verify Worker", deletes this criterion, drops FRAUD-05/06 from the Requirements line, and removes the rescinded `duration_seconds: 0` tolerance note (D-03a). The §v2 SEARCH-V2-01 hybrid-search layer / iOS / Play-Store rollout are parked in REQUIREMENTS.md §v2.
