...
export async function bootstrapSecrets(): Promise<void> {
  ...
  console.log(
    `[SecretsLoader] APP_NAME = "${secretName}". Fetching secrets from AWS Secrets Manager in ${region}...`,
  );
  ...
  try {
    ...
    if (response.SecretString) {
      ...
      console.log(
        `[SecretsLoader] Successfully loaded ${loadedCount} environment variables from Secrets Manager.`,
      );
    } else {
      console.warn(
        `[SecretsLoader] Secrets Manager returned an empty payload for secret: ${secretName}`,
      );
    }
  } catch (error) {
    console.error(
      `[SecretsLoader] CRITICAL: Failed to load secrets from AWS Secrets Manager:`,
      error,
    );
