...
-- must be removed first; their dependents (recording_segment_parts,
-- recordings_to_verify, recording_events_outbox) are ON DELETE CASCADE and go
-- with them. Idempotent: both DELETEs are no-ops on a DB that never ran the
